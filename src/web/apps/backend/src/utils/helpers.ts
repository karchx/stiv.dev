 ...
 */
export const parseErrorSchema = (error: Yup.ValidationError, validateAllFieldCriteria: boolean) => {
...
const appendErrors = (name: string, validateAllFieldCriteria: boolean, errors: Record<any, any>, type: string, message: string[] | string) =>
  validateAllFieldCriteria
    ? {
      ...errors[name],
      types: {
...
export const mapErrors: any = (errors: any[]) => {
  return errors.reduce(
    (acc, curr) => ({
      ...acc,
      [curr.property]: curr.children?.length > 0 ? mapErrors(curr.children) : Object.values(curr.constraints ?? {}),
    }),
    {},
  );
};
...
export const omitForModel = <Ob extends Record<any, any>>(obj: Ob, keys: (keyof Ob)[]): Ob => {
